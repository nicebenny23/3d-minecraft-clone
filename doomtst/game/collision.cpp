...
bool collision::boxCollidesWithEntity(geo::Box blk, HitQuery query) {
...
stn::Option<Vec3> colideentandblock(Collider& entity, block& tocollide, bool is_trigger) {
	if (!tocollide.owner().has_component<aabb::Collider>()) {
		return zerov;
	}
