...
struct slimemove : ecs::component {
	...
	void update() {
		...
		if (gotopos != zerov) {
			...
			owner().get_component<rigidbody>().velocity += v3::Vec3(normal(gotopos).x, 0, normal(gotopos).z) * world().get_resource<timename::TimeManager>().dt;
...
inline ecs::obj createslime(v3::Coord pos, bool type, ecs::Ecs& world) {

	ecs::obj refmodel = world.spawn_empty();
