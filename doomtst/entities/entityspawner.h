...
inline bool should_despawn(v3::Point3 pos, v3::Point3 player) {
	...
	return 46< v3::dist(pos, player);

}
inline bool spawnable_dist(v3::Point3 pos, v3::Point3 player) {

	return !should_despawn(pos, player) &&10< v3::dist(pos, player);
...
struct spawn_mobs :ecs::System {
	...
	stn::Option<geo::Box> find_spawn_location(v3::Vec3 Scl, grid::Grid& grid) {
		...
		for (size_t chks = 0; chks < max_checks; chks++) {
			...
			stn::Option<geo::Box> spawn_loc = voxtra::findground(unit_scale,grid);
			if (!spawn_loc) {
				return stn::None;
			}
	...
	void run(ecs::Ecs& ecs) {
		...
		ecs::View< slimes::SlimePathFinder,ecs::world_transform> slimes(ecs);
		for (auto [slime,transform] : slimes) {

			if (should_despawn(transform.transform.position, player::player_for(ecs).get_component<ecs::world_transform>().transform.position)) {
				slime.owner().destroy();
			}
			total_alive++;
		}
		...
		for (auto [player] : players) {
			{
				if (randomnum < spawnthreshold) {
					...
					stn::Option<geo::Box> spawn_loc = find_spawn_location(unitv,ecs.get_resource<grid::Grid>());
					if (!spawn_loc) {
						continue;
					}
