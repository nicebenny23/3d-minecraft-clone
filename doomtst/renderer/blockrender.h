...
namespace blockrender {
	...
	inline bool chunk_viewable(Chunk::chunk& chk) {
		...
		geo::Box chkb = geo::Box(chk.center(), unit_scale * float(Chunk::chunklength) / 2.f);
	...
	void emit_face(int face, const blockmesh& torender, renderer::MeshData& mesh) {
	...
	void emit_block(blockmesh& torender, renderer::MeshData& mesh) {
		if (torender.render_id != minecraftair) {
			for (int i = 0; i < 6; i++) {
	...
	void renderchunk(Chunk::chunkmesh& mesh) {
			...
			for (int i = 0; i < mesh.faces.length(); i++) {
				emit_face(mesh.faces[i].facenum.ind(), *(mesh.faces[i].mesh), mesh_data);
			}
