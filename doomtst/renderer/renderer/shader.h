...
struct shader
{
	...
	void seti(int val, const char* name)
	{
	...
	void SetVector3f(v3::Vec3 val, const char* name)
	{
		...
		glad_glUniform3f(uniformlocation(name), GLfloat(val.x), GLfloat(val.y), GLfloat(val.z));
	...
	void SetVector2f(v2::Vec2 val, const char* name)
	{
		...
		glad_glUniform2f(uniformlocation(name), GLfloat(val.x),GLfloat( val.y));
	}
