...
namespace v3 {
	...
	struct Vec3
	{
		...
		bool is_nan() const { return isnan(x) || isnan(y) || isnan(z); }
		bool throw_if_nan() const {
			if (is_nan()) { throw std::logic_error("Vec3 Must not be NaN"); }
		}

		void operator=(const Vec3& p1) { x = p1.x; y = p1.y; z = p1.z; }
		bool operator==(const Vec3& p1) { return apxf(x, p1.x) && apxf(y, p1.y) && apxf(z, p1.z); }
		bool operator!=(const Vec3& p1) { return !apxf(x, p1.x) || !apxf(y, p1.y) || !apxf(z, p1.z); }

		Vec3 operator+(const Vec3& p1) const { return Vec3(x + p1.x, y + p1.y, z + p1.z); }
		Vec3& operator+=(const Vec3& p1) { x += p1.x; y += p1.y; z += p1.z; return *this; }

		Vec3 operator-(const Vec3& p1) const { return Vec3(x - p1.x, y - p1.y, z - p1.z); }
		Vec3& operator-=(const Vec3& p1) { x -= p1.x; y -= p1.y; z -= p1.z; return *this; }

		
		Vec3 operator*(const Vec3& scale) const { return Vec3(x * scale.x, y * scale.y, z * scale.z); }
		...
		v2::Vec2 xy() const { return v2::Vec2(x, y); }
		v2::Vec2 xz() const { return v2::Vec2(x, z); }
		v2::Vec2 yz() const { return v2::Vec2(y, z); }

		Vec3 operator/(double scale) const {
			if (scale == 0) { throw std::logic_error("Unable to divide a Vec3 by zero"); }
		...
		inline double length() const{ return sqrt(mag2()); }
	...
	inline bool is_nan(v3::Vec3 vec) { return vec.is_nan(); }
	inline bool throw_if_nan(v3::Vec3 vec) { vec.throw_if_nan(); }
	...
	inline double dist(const Vec3& p, const Vec3& p1) { return (sqrt(dist2(p, p1))); }
	inline double dot(const Vec3& p, const Vec3& p1) { return (p.x * p1.x + p.y * p1.y + p.z * p1.z); }
	inline double mag2(const Vec3& p) { return ((p.x * p.x + p.y * p.y + p.z * p.z)); }
	inline Vec3 normal(const Vec3& p) {
		float mt = p.length();
		if (mt == 0) { throw std::logic_error("Cannot Normalize the zero vector"); }
		return (p / mt);
	}
	//norm
	inline Vec3 zero_fixed_normal(const Vec3& p) {
		float mt = p.length();
		if (mt == 0) { return zerov; }
		return (p / mt);
	}
	...
	inline Vec3 operator+(const Vec3& a, const Coord& b) {
		return Vec3(a.x + b.x, a.y + b.y, a.z + b.z);
	}
	inline Vec3 operator+(const Coord& a, const Vec3& b) {
		return Vec3(a.x + b.x, a.y + b.y, a.z + b.z);
	}
	inline Vec3 operator-(const Coord& a, const Vec3& b) {
		return Vec3(a.x - b.x, a.y - b.y, a.z - b.z);
	...
	inline Vec3 operator*(float scale, const Vec3& vec) { return vec * scale; }
