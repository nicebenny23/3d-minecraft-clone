...
namespace stn {
	...
	private:
		...
		constexpr List(constructor_tag , F&& f, std::index_sequence<I...>) : storage{ static_cast<T>(f(I))... } {
		}

	public:
		template<typename F>
			requires std::invocable<F&, size_t>&& std::constructible_from<T, std::invoke_result_t<F&, size_t>>
		constexpr List(F&& f) : List(constructor_tag(),std::forward<F>(f), std::make_index_sequence<N>{}) {
		}
