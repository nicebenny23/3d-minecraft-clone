...
namespace stn {
	...
	concept Range = requires(RangeType & r) {
		...
		{
			std::end(r)
		}   -> std::sentinel_for<decltype(std::begin(r))>;
	};
	...
	template<Range Source, typename Func>
		requires requires(Func f, std::iter_value_t<std::ranges::iterator_t<Source&>> const& elem) {
			{
				f(elem)
			} -> std::same_as<void>; // func must accept const ref and return void
	}struct InspectView;
	template<Range Source, typename Func>
	struct MapView;
	...
	struct range {
		...
		iterator begin() {
			return std::begin(stored_range);
		}
		iterator end() {
			return std::end(stored_range);
		}
	...
	struct MapView {
		...
		struct Iterator {
			...
			bool operator==(Iterator<OtherIter, OtherFuncPtr> const& o) const {
				return iter == o.iter;
			}
			template<typename OtherIter, typename OtherFuncPtr>
			bool operator!=(Iterator<OtherIter, OtherFuncPtr> const& o) const {
			...
			//random access
			reference operator[](difference_type n) const requires is_random_access {
				return std::invoke(*func_ptr, iter[n]);
			}
			Iterator& operator+=(difference_type n) requires is_random_access {
				iter += n; return *this;
			}
			Iterator& operator-=(difference_type n) requires is_random_access {
				iter -= n; return *this;
			}
			Iterator operator+(difference_type n) const requires is_random_access {
				return Iterator{ iter + n, func_ptr };
			}
			Iterator operator-(difference_type n) const requires is_random_access {
				return Iterator{ iter - n, func_ptr };
			}
			difference_type operator-(const Iterator& other) const requires is_random_access {
		...
		iterator begin() {
			return iterator{ std::begin(source), &func };
		}
		iterator end() {
			return iterator{ std::end(source), &func };
		}
		const_iterator begin() const {
			return const_iterator{ std::begin(source), &func };
		}
		const_iterator end()   const {
			return const_iterator{ std::end(source), &func };
		}
	};
	...
	struct FilterView {
		...
		struct Iterator {
			...
			Iterator& operator++() {
				do {
					++curr;
				} while (curr != end && !std::invoke(*pred_ptr, *curr));
				return *this;
			}
			Iterator operator++(int) {
				Iterator tmp = *this; ++(*this); return tmp;
			...
			bool operator==(Iterator<OIter, OPredType> const& other) const {
		...
		iterator begin() {
			return iterator{ std::begin(source), std::end(source), &pred };
		}
		iterator end() {
			return iterator{ std::end(source), std::end(source), &pred };
		}
		const_iterator begin() const {
			return const_iterator{ std::begin(source), std::end(source), &pred };
		}
		const_iterator end()   const {
			return const_iterator{ std::end(source), std::end(source), &pred };
		}
	};
	...
	struct EnumView {
		...
		struct Iterator {
			...
			bool operator==(Iterator<OtherIter> const& o) const {
				return iter == o.iter;
			}
			template<typename OtherIter>
			bool operator!=(Iterator<OtherIter> const& o) const {
			...
			Iterator& operator+=(difference_type n) requires is_random_access {
				iter += n; index += n; return *this;
			}
			Iterator& operator-=(difference_type n) requires is_random_access {
				iter -= n; index -= n; return *this;
			}
			Iterator operator+(difference_type n) const requires is_random_access {
				return Iterator{ iter + n, index + n };
			}
			Iterator operator-(difference_type n) const requires is_random_access {
				return Iterator{ iter - n, index - n };
			}
			difference_type operator-(const Iterator& other) const requires is_random_access {
				return Iterator{ iter - other.iter, index - other.index };
			}
		...
		iterator end() {
			return iterator{ std::end(source) };
		}
		const_iterator begin() const {
			return const_iterator{ std::begin(source) };
		}
		const_iterator end()   const {
			return const_iterator{ std::end(source) };
		}
	};
	template<typename R>
	...
	using KeyResult= std::invoke_result_t<Key, std::ranges::range_value_t<Range>>;
	
		template<typename Range,typename Key>
		concept KeySortable = Sortable<KeyResult<Range, Key>>;

	template<RandomRange R, typename CompareKey> requires KeySortable<R,CompareKey>
	struct SortByKeyAction {
		R& src;
		CompareKey key;
		explicit SortByKeyAction(R& r, CompareKey comp_key = {}) : src(r), key(comp_key) {
			std::sort(std::begin(src), std::end(src), [&](const auto& a, const auto& b) {return key(a) < key(b);});
		}
	...
	struct SortAdaptor {
		constexpr auto operator()() const {
			return [](auto&& r) {
				...
				return SortByKeyAction<R, std::identity>(r,std::identity());
				};
		}

		template<typename Key>
		constexpr auto operator()(Key comp_key) const {
			return [comp_key](auto&& r) {

				using T = std::remove_reference_t<decltype(r)>;
				static_assert(KeySortable<T, Key>,
					"Key must return values comparable with <");
				return SortByKeyAction<T, Key>(r, comp_key);
			};
		}
	};
